//#region ../../node_modules/.pnpm/version-polling@1.3.3/node_modules/version-polling/dist/index.js
/*!
...
var VersionPolling = class {
	start() {
		...
		this.worker = createWorker(() => {
			...
			let globalData;
			let versionControl;
			const versionControlMap = /* @__PURE__ */ new Map();
			...
			versionControlMap.set("chunkHash", {
				...
				fetchChunkHash: () => {
					if (!globalData.htmlFileUrl) throw new Error("[version-polling]: htmlFileUrl is required");
					return fetch(`${globalData.htmlFileUrl}?t=${+/* @__PURE__ */ new Date()}`).then((response) => response.text()).then((response) => {
			...
			versionControlMap.set("versionJson", {
				...
				fetchVersionFile: () => {
					...
					return fetch(`${globalData.versionFileUrl}?t=${+/* @__PURE__ */ new Date()}`).then((response) => response.json()).then((response) => {
	...
	constructor(options) {
		...
		_define_property(this, "visibilityHandler", () => {
			if ("visible" === document.visibilityState) {
				var _this_worker;
				null === (_this_worker = this.worker) || void 0 === _this_worker || _this_worker.postMessage({ code: "resume" });
			} else {
				var _this_worker1;
				null === (_this_worker1 = this.worker) || void 0 === _this_worker1 || _this_worker1.postMessage({ code: "pause" });
			}
		});
		_define_property(this, "eventHandler", () => {
			var _this_worker;
			null === (_this_worker = this.worker) || void 0 === _this_worker || _this_worker.postMessage({ code: "check" });
		});
		this.options = {
			...DEFAULT_OPTIONS,
			...options
...
function createVersionPolling(options) {
	return new VersionPolling(options);
}
//#endregion
export { VersionPolling, createVersionPolling };

//# sourceMappingURL=version-polling.js.map