import { Bt as onUnmounted, Gn as reactive, It as onMounted, Ln as isReactive, Tt as isVNode, Xn as shallowRef, _n as watch, dt as getCurrentInstance, i as createApp, kt as nextTick, mt as h, qn as ref, rr as unref, rt as defineComponent, zn as isRef } from "./runtime-dom.esm-bundler-BEW9uiVc.js";
import "./vue.js";
//#region ../../node_modules/.pnpm/vue-tippy@6.7.1_vue@3.5.33_typescript@5.9.3_/node_modules/vue-tippy/dist/vue-tippy.esm-browser.js
/*!
* vue-tippy v6.7.1
* (c) 2025 
* @license MIT
...
function createArrowElement(value) {
	...
	if (value === true) arrow.className = ARROW_CLASS;
		...
		if (isElement$1(value)) arrow.appendChild(value);
...
var hideAll = function hideAll(_temp) {
	...
	mountedInstances.forEach(function(instance) {
		...
		if (!isExcluded) {
			...
			instance.hide();
			if (!instance.state.isDestroyed) instance.setProps({ duration: originalDuration });
		}
	});
};
var applyStylesModifier = Object.assign({}, applyStyles$1, { effect: function effect(_ref) {
	var state = _ref.state;
	var initialStyles = {
		popper: {
			position: state.options.strategy,
...
var followCursor = {
	...
	fn: function fn(instance) {
		...
		function onMouseMove(event) {
			...
			if (isCursorOverReference || !instance.props.interactive) instance.setProps({ getReferenceClientRect: function getReferenceClientRect() {
				var rect = reference.getBoundingClientRect();
				var x = clientX;
				var y = clientY;
...
function getProps(props, modifier) {
	...
	return { popperOptions: Object.assign({}, props.popperOptions, { modifiers: [].concat((((_props$popperOptions = props.popperOptions) == null ? void 0 : _props$popperOptions.modifiers) || []).filter(function(_ref) {
			...
			fn: function fn(_ref2) {
				...
				if (isEnabled()) {
					...
					if (placement !== state.placement && triedPlacements.indexOf(state.placement) === -1) {
						...
						instance.setProps({ getReferenceClientRect: function getReferenceClientRect() {
...
tippy.setDefaultProps({ onShow: (instance) => {
...
function useTippy(el, opts = {}, settings = {
	...
	const getContent = (content) => {
		...
		if (isVNode(unwrappedContent)) {
			if (!headlessApp.value) {
				...
				if (vm) Object.assign(headlessApp.value._context, vm.appContext);
			...
			newContent = () => getContainer();
		} else newContent = unwrappedContent;
		return newContent;
	};
	const getProps = (opts) => {
		let options = {};
		if (isRef(opts)) options = opts.value || {};
		else if (isReactive(opts)) options = { ...opts };
		else options = { ...opts };
		if (options.content) options.content = getContent(options.content);
		if (options.triggerTarget) options.triggerTarget = isRef(options.triggerTarget) ? options.triggerTarget.value : options.triggerTarget;
		if (!options.plugins || !Array.isArray(options.plugins)) options.plugins = [];
	...
	const setContent = (value) => {
		var _a;
		(_a = instance.value) === null || _a === void 0 || _a.setContent(getContent(value));
	};
	const setProps = (value) => {
		var _a;
		(_a = instance.value) === null || _a === void 0 || _a.setProps(getProps(value));
	};
	const destroy = () => {
		var _a;
		if (instance.value) {
		...
		container = null;
		(_a = headlessApp.value) === null || _a === void 0 || _a.unmount();
		headlessApp.value = void 0;
	};
	const show = () => {
		var _a;
		(_a = instance.value) === null || _a === void 0 || _a.show();
	};
	const hide = () => {
		var _a;
		(_a = instance.value) === null || _a === void 0 || _a.hide();
	};
	const disable = () => {
		var _a;
		(_a = instance.value) === null || _a === void 0 || _a.disable();
		state.value.isEnabled = false;
	};
	const enable = () => {
		var _a;
		(_a = instance.value) === null || _a === void 0 || _a.enable();
		state.value.isEnabled = true;
	};
	const unmount = () => {
		var _a;
		(_a = instance.value) === null || _a === void 0 || _a.unmount();
	};
	const mount = () => {
		if (!el) return;
		let target = isRef(el) ? el.value : el;
		if (typeof target === "function") target = target();
		if (isComponentInstance(target)) target = target.$el;
		if (target) {
	...
	if (settings.mount) if (vm) if (vm.isMounted) mount();
	else onMounted(mount);
	else mount();
	if (vm) onUnmounted(() => {
		destroy();
	});
	if (isRef(opts) || isReactive(opts)) watch(opts, refresh, { immediate: false });
	else if (isRef(opts.content)) watch(opts.content, refreshContent, { immediate: false });
	return response;
}
...
function useSingleton(instances, optionalProps) {
	const singleton = ref();
	onMounted(() => {
		singleton.value = createSingleton((Array.isArray(instances) ? instances.map((i) => i.value) : typeof instances === "function" ? instances() : instances.value).map((instance) => {
			if (instance instanceof Element) return instance._tippy;
			return instance;
		}).filter(Boolean), optionalProps ? {
			allowHTML: true,
			...optionalProps
		} : { allowHTML: true });
	});
	...
	setup(props, { slots, emit, expose }) {
		...
		const getOptions = () => {
			...
			for (const prop of [
				...
				"contentTag",
				"contentClass"
			]) if (options.hasOwnProperty(prop)) delete options[prop];
			return options;
		};
		let target = () => unrefElement(elem);
		if (props.to) {
			if (typeof Element !== "undefined" && props.to instanceof Element) target = () => props.to;
			else if (props.to === "parent") target = () => {
				let el = elem.value;
				if (!el) el = elem.value = findParentHelper.value.parentElement;
				return el;
			};
			else if (typeof props.to === "string" || props.to instanceof String) target = () => document.querySelector(props.to);
		}
		const tippy = useTippy(target, getOptions());
		let contentSlot = slots.content;
		if (!contentSlot && props.to === "parent") contentSlot = slots.default;
		onMounted(() => {
			mounted.value = true;
			nextTick(() => {
				if (contentSlot) tippy.setContent(() => contentElem.value);
			});
		});
		watch(tippy.state, () => {
		...
		return () => {
			...
			return h(typeof props.tag === "string" ? props.tag : props.tag, {
				ref: elem,
				"data-v-tippy": ""
...
var TippySingleton = defineComponent({
	...
	mounted() {
		var _a;
		const elements = this.$el.parentElement.querySelectorAll("[data-v-tippy]");
		this.instances = Array.from(elements).map((el) => el._tippy).filter(Boolean);
		(_a = this.singleton) === null || _a === void 0 || _a.setInstances(this.instances);
	},
	render() {
...
var directive = {
	mounted(el, binding, vnode) {
		...
		if (placement) opts.placement = opts.placement || placement;
		if (withArrow) opts.arrow = opts.arrow !== void 0 ? opts.arrow : true;
		if (vnode.props && vnode.props.onTippyShow) opts.onShow = function(...args) {
			var _a;
			return (_a = vnode.props) === null || _a === void 0 ? void 0 : _a.onTippyShow(...args);
		};
		if (vnode.props && vnode.props.onTippyShown) opts.onShown = function(...args) {
			var _a;
			return (_a = vnode.props) === null || _a === void 0 ? void 0 : _a.onTippyShown(...args);
		};
		if (vnode.props && vnode.props.onTippyHidden) opts.onHidden = function(...args) {
			var _a;
			return (_a = vnode.props) === null || _a === void 0 ? void 0 : _a.onTippyHidden(...args);
		};
		if (vnode.props && vnode.props.onTippyHide) opts.onHide = function(...args) {
			var _a;
			return (_a = vnode.props) === null || _a === void 0 ? void 0 : _a.onTippyHide(...args);
		};
		if (vnode.props && vnode.props.onTippyMount) opts.onMount = function(...args) {
			var _a;
		...
		if (el.getAttribute("content") && !opts.content) opts.content = el.getAttribute("content");
		useTippy(el, opts);
	},
	unmounted(el) {
		if (el.$tippy) el.$tippy.destroy();
		else if (el._tippy) el._tippy.destroy();
	},
	updated(el, binding) {
		const opts = typeof binding.value === "string" ? { content: binding.value } : binding.value || {};
		...
		if (el.getAttribute("content") && !opts.content) opts.content = el.getAttribute("content");
		if (el.$tippy) el.$tippy.setProps(opts || {});
		else if (el._tippy) el._tippy.setProps(opts || {});
	}
};
var plugin = { install(app, options = {}) {
...
//#endregion
export { TippyComponent as Tippy, TippySingleton, plugin as default, plugin, directive, hideAll, ROUND_ARROW as roundArrow, setDefaultProps$1 as setDefaultProps, tippy, useSingleton, useTippy, useTippyComponent };
