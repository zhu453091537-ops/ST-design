...
async function exportJSONPackage(iconSet, options) {
	...
	iconsKeys.forEach((attr) => {
		if (exportedJSON[attr] !== void 0) icons[attr] = exportedJSON[attr];
	});
	const metadata = {};
	...
	for (const key in contents) {
		...
		if (data !== void 0) {
			...
			mjsImports.push(`import ${attr} from '${relativeFile}' with { type: 'json' };`);
