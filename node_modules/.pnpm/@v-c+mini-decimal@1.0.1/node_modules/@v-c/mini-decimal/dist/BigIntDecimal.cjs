...
var BigIntDecimal = class BigIntDecimal {
	...
	nan = false;
	constructor(value) {
		if (require_numberUtil.isEmpty(value)) {
			this.empty = true;
			return;
		}
		...
		if (require_numberUtil.isE(mergedValue)) mergedValue = Number(mergedValue);
		mergedValue = typeof mergedValue === "string" ? mergedValue : require_numberUtil.num2str(mergedValue);
		if (require_numberUtil.validateNumber(mergedValue)) {
			const trimRet = require_numberUtil.trimNumber(mergedValue);
	...
	cal(offset, calculator, calDecimalLen) {
		...
		const { negativeStr, trimStr } = require_numberUtil.trimNumber(valueStr);
	...
	toString(safe = true) {
		...
		return require_numberUtil.trimNumber(`${this.getMark()}${this.getIntegerStr()}.${this.getDecimalStr()}`).fullStr;
	}
};
