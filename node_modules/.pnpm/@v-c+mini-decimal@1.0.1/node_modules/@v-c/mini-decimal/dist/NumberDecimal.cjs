...
var NumberDecimal = class NumberDecimal {
	...
	add(value) {
		...
		const maxPrecision = Math.max(require_numberUtil.getNumberPrecision(this.number), require_numberUtil.getNumberPrecision(target));
	...
	toString(safe = true) {
		...
		return require_numberUtil.num2str(this.number);
	}
};
