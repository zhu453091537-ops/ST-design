import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import KeyOutlinedSvg from "@ant-design/icons-svg/es/asn/KeyOutlined.js";

//#region src/icons/KeyOutlined.tsx
/**![key](data:image/svg+xml;base64,) */
const KeyOutlined = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": KeyOutlinedSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "KeyOutlined"
});

//#endregion
export { KeyOutlined as default };