import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import DatabaseTwoToneSvg from "@ant-design/icons-svg/es/asn/DatabaseTwoTone.js";

//#region src/icons/DatabaseTwoTone.tsx
/**![database](data:image/svg+xml;base64,) */
const DatabaseTwoTone = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": DatabaseTwoToneSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "DatabaseTwoTone"
});

//#endregion
export { DatabaseTwoTone as default };