import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import CodepenOutlinedSvg from "@ant-design/icons-svg/es/asn/CodepenOutlined.js";

//#region src/icons/CodepenOutlined.tsx
/**![codepen](data:image/svg+xml;base64,) */
const CodepenOutlined = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": CodepenOutlinedSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "CodepenOutlined"
});

//#endregion
export { CodepenOutlined as default };