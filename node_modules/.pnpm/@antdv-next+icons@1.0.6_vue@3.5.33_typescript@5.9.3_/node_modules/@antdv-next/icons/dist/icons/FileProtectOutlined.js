import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import FileProtectOutlinedSvg from "@ant-design/icons-svg/es/asn/FileProtectOutlined.js";

//#region src/icons/FileProtectOutlined.tsx
/**![file-protect](data:image/svg+xml;base64,) */
const FileProtectOutlined = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": FileProtectOutlinedSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "FileProtectOutlined"
});

//#endregion
export { FileProtectOutlined as default };