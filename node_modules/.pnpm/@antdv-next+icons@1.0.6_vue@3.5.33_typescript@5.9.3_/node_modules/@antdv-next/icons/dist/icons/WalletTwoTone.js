import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import WalletTwoToneSvg from "@ant-design/icons-svg/es/asn/WalletTwoTone.js";

//#region src/icons/WalletTwoTone.tsx
/**![wallet](data:image/svg+xml;base64,) */
const WalletTwoTone = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": WalletTwoToneSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "WalletTwoTone"
});

//#endregion
export { WalletTwoTone as default };