import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import PushpinTwoToneSvg from "@ant-design/icons-svg/es/asn/PushpinTwoTone.js";

//#region src/icons/PushpinTwoTone.tsx
/**![pushpin](data:image/svg+xml;base64,) */
const PushpinTwoTone = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": PushpinTwoToneSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "PushpinTwoTone"
});

//#endregion
export { PushpinTwoTone as default };