import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import MinusCircleFilledSvg from "@ant-design/icons-svg/es/asn/MinusCircleFilled.js";

//#region src/icons/MinusCircleFilled.tsx
/**![minus-circle](data:image/svg+xml;base64,) */
const MinusCircleFilled = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": MinusCircleFilledSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "MinusCircleFilled"
});

//#endregion
export { MinusCircleFilled as default };