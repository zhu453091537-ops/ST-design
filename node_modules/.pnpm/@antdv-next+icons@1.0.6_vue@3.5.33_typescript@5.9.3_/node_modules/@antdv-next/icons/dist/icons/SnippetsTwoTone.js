import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import SnippetsTwoToneSvg from "@ant-design/icons-svg/es/asn/SnippetsTwoTone.js";

//#region src/icons/SnippetsTwoTone.tsx
/**![snippets](data:image/svg+xml;base64,) */
const SnippetsTwoTone = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": SnippetsTwoToneSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "SnippetsTwoTone"
});

//#endregion
export { SnippetsTwoTone as default };