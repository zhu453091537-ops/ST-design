import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import InstagramFilledSvg from "@ant-design/icons-svg/es/asn/InstagramFilled.js";

//#region src/icons/InstagramFilled.tsx
/**![instagram](data:image/svg+xml;base64,) */
const InstagramFilled = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": InstagramFilledSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "InstagramFilled"
});

//#endregion
export { InstagramFilled as default };