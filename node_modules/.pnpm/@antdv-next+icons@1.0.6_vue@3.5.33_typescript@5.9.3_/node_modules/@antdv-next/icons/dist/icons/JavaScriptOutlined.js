import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import JavaScriptOutlinedSvg from "@ant-design/icons-svg/es/asn/JavaScriptOutlined.js";

//#region src/icons/JavaScriptOutlined.tsx
/**![java-script](data:image/svg+xml;base64,) */
const JavaScriptOutlined = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": JavaScriptOutlinedSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "JavaScriptOutlined"
});

//#endregion
export { JavaScriptOutlined as default };