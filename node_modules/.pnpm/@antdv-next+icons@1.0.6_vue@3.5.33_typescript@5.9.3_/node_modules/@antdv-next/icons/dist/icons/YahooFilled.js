import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import YahooFilledSvg from "@ant-design/icons-svg/es/asn/YahooFilled.js";

//#region src/icons/YahooFilled.tsx
/**![yahoo](data:image/svg+xml;base64,) */
const YahooFilled = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": YahooFilledSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "YahooFilled"
});

//#endregion
export { YahooFilled as default };