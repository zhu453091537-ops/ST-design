import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import AreaChartOutlinedSvg from "@ant-design/icons-svg/es/asn/AreaChartOutlined.js";

//#region src/icons/AreaChartOutlined.tsx
/**![area-chart](data:image/svg+xml;base64,) */
const AreaChartOutlined = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": AreaChartOutlinedSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "AreaChartOutlined"
});

//#endregion
export { AreaChartOutlined as default };