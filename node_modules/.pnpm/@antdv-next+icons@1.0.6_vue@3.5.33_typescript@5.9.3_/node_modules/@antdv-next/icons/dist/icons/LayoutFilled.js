import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import LayoutFilledSvg from "@ant-design/icons-svg/es/asn/LayoutFilled.js";

//#region src/icons/LayoutFilled.tsx
/**![layout](data:image/svg+xml;base64,) */
const LayoutFilled = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": LayoutFilledSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "LayoutFilled"
});

//#endregion
export { LayoutFilled as default };