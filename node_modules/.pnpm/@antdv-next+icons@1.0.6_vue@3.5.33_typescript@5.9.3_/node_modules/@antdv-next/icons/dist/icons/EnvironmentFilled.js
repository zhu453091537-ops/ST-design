import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import EnvironmentFilledSvg from "@ant-design/icons-svg/es/asn/EnvironmentFilled.js";

//#region src/icons/EnvironmentFilled.tsx
/**![environment](data:image/svg+xml;base64,) */
const EnvironmentFilled = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": EnvironmentFilledSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "EnvironmentFilled"
});

//#endregion
export { EnvironmentFilled as default };