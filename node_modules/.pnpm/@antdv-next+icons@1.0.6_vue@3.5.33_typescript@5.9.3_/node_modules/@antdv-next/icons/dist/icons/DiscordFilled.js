import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import DiscordFilledSvg from "@ant-design/icons-svg/es/asn/DiscordFilled.js";

//#region src/icons/DiscordFilled.tsx
/**![discord](data:image/svg+xml;base64,) */
const DiscordFilled = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": DiscordFilledSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "DiscordFilled"
});

//#endregion
export { DiscordFilled as default };