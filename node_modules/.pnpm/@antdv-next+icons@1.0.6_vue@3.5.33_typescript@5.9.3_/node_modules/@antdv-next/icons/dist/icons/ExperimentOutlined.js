import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import ExperimentOutlinedSvg from "@ant-design/icons-svg/es/asn/ExperimentOutlined.js";

//#region src/icons/ExperimentOutlined.tsx
/**![experiment](data:image/svg+xml;base64,) */
const ExperimentOutlined = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": ExperimentOutlinedSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "ExperimentOutlined"
});

//#endregion
export { ExperimentOutlined as default };