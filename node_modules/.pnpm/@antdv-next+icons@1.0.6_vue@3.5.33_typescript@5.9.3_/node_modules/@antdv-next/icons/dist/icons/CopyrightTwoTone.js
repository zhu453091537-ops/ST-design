import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import CopyrightTwoToneSvg from "@ant-design/icons-svg/es/asn/CopyrightTwoTone.js";

//#region src/icons/CopyrightTwoTone.tsx
/**![copyright](data:image/svg+xml;base64,) */
const CopyrightTwoTone = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": CopyrightTwoToneSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "CopyrightTwoTone"
});

//#endregion
export { CopyrightTwoTone as default };