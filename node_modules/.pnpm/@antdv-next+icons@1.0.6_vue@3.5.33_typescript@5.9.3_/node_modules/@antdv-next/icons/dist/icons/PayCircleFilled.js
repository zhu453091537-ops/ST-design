import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import PayCircleFilledSvg from "@ant-design/icons-svg/es/asn/PayCircleFilled.js";

//#region src/icons/PayCircleFilled.tsx
/**![pay-circle](data:image/svg+xml;base64,) */
const PayCircleFilled = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": PayCircleFilledSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "PayCircleFilled"
});

//#endregion
export { PayCircleFilled as default };