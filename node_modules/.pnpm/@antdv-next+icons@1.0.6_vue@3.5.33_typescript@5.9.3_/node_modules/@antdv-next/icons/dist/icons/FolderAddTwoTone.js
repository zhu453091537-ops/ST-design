import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import FolderAddTwoToneSvg from "@ant-design/icons-svg/es/asn/FolderAddTwoTone.js";

//#region src/icons/FolderAddTwoTone.tsx
/**![folder-add](data:image/svg+xml;base64,) */
const FolderAddTwoTone = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": FolderAddTwoToneSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "FolderAddTwoTone"
});

//#endregion
export { FolderAddTwoTone as default };