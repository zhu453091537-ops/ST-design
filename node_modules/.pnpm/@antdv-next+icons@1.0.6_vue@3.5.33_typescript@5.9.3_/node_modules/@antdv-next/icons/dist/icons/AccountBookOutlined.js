import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import AccountBookOutlinedSvg from "@ant-design/icons-svg/es/asn/AccountBookOutlined.js";

//#region src/icons/AccountBookOutlined.tsx
/**![account-book](data:image/svg+xml;base64,) */
const AccountBookOutlined = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": AccountBookOutlinedSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "AccountBookOutlined"
});

//#endregion
export { AccountBookOutlined as default };