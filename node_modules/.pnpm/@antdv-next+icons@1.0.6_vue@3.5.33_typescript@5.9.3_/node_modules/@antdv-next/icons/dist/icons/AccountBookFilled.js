import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import AccountBookFilledSvg from "@ant-design/icons-svg/es/asn/AccountBookFilled.js";

//#region src/icons/AccountBookFilled.tsx
/**![account-book](data:image/svg+xml;base64,) */
const AccountBookFilled = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": AccountBookFilledSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "AccountBookFilled"
});

//#endregion
export { AccountBookFilled as default };