import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import HighlightFilledSvg from "@ant-design/icons-svg/es/asn/HighlightFilled.js";

//#region src/icons/HighlightFilled.tsx
/**![highlight](data:image/svg+xml;base64,) */
const HighlightFilled = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": HighlightFilledSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "HighlightFilled"
});

//#endregion
export { HighlightFilled as default };