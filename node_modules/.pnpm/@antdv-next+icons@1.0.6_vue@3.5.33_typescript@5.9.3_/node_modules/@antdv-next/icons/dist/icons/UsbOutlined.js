import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import UsbOutlinedSvg from "@ant-design/icons-svg/es/asn/UsbOutlined.js";

//#region src/icons/UsbOutlined.tsx
/**![usb](data:image/svg+xml;base64,) */
const UsbOutlined = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": UsbOutlinedSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "UsbOutlined"
});

//#endregion
export { UsbOutlined as default };