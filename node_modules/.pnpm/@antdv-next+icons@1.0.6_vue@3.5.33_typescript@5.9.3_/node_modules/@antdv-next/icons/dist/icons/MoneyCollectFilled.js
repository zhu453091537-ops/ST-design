import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import MoneyCollectFilledSvg from "@ant-design/icons-svg/es/asn/MoneyCollectFilled.js";

//#region src/icons/MoneyCollectFilled.tsx
/**![money-collect](data:image/svg+xml;base64,) */
const MoneyCollectFilled = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": MoneyCollectFilledSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "MoneyCollectFilled"
});

//#endregion
export { MoneyCollectFilled as default };