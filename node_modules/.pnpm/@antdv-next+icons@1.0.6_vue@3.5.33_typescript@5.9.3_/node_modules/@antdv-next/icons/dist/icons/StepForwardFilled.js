import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import StepForwardFilledSvg from "@ant-design/icons-svg/es/asn/StepForwardFilled.js";

//#region src/icons/StepForwardFilled.tsx
/**![step-forward](data:image/svg+xml;base64,) */
const StepForwardFilled = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": StepForwardFilledSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "StepForwardFilled"
});

//#endregion
export { StepForwardFilled as default };