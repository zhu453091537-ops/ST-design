import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import WechatWorkOutlinedSvg from "@ant-design/icons-svg/es/asn/WechatWorkOutlined.js";

//#region src/icons/WechatWorkOutlined.tsx
/**![wechat-work](data:image/svg+xml;base64,) */
const WechatWorkOutlined = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": WechatWorkOutlinedSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "WechatWorkOutlined"
});

//#endregion
export { WechatWorkOutlined as default };