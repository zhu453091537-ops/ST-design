import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import ChromeFilledSvg from "@ant-design/icons-svg/es/asn/ChromeFilled.js";

//#region src/icons/ChromeFilled.tsx
/**![chrome](data:image/svg+xml;base64,) */
const ChromeFilled = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": ChromeFilledSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "ChromeFilled"
});

//#endregion
export { ChromeFilled as default };