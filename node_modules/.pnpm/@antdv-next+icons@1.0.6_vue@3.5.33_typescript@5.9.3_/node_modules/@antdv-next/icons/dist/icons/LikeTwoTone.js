import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import LikeTwoToneSvg from "@ant-design/icons-svg/es/asn/LikeTwoTone.js";

//#region src/icons/LikeTwoTone.tsx
/**![like](data:image/svg+xml;base64,) */
const LikeTwoTone = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": LikeTwoToneSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "LikeTwoTone"
});

//#endregion
export { LikeTwoTone as default };