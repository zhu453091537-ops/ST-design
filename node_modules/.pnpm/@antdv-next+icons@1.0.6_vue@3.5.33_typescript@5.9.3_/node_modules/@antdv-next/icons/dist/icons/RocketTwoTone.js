import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import RocketTwoToneSvg from "@ant-design/icons-svg/es/asn/RocketTwoTone.js";

//#region src/icons/RocketTwoTone.tsx
/**![rocket](data:image/svg+xml;base64,) */
const RocketTwoTone = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": RocketTwoToneSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "RocketTwoTone"
});

//#endregion
export { RocketTwoTone as default };