import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import QuestionCircleFilledSvg from "@ant-design/icons-svg/es/asn/QuestionCircleFilled.js";

//#region src/icons/QuestionCircleFilled.tsx
/**![question-circle](data:image/svg+xml;base64,) */
const QuestionCircleFilled = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": QuestionCircleFilledSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "QuestionCircleFilled"
});

//#endregion
export { QuestionCircleFilled as default };