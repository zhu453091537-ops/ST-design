import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import UnlockTwoToneSvg from "@ant-design/icons-svg/es/asn/UnlockTwoTone.js";

//#region src/icons/UnlockTwoTone.tsx
/**![unlock](data:image/svg+xml;base64,) */
const UnlockTwoTone = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": UnlockTwoToneSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "UnlockTwoTone"
});

//#endregion
export { UnlockTwoTone as default };