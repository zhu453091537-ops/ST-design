import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import WeiboOutlinedSvg from "@ant-design/icons-svg/es/asn/WeiboOutlined.js";

//#region src/icons/WeiboOutlined.tsx
/**![weibo](data:image/svg+xml;base64,) */
const WeiboOutlined = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": WeiboOutlinedSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "WeiboOutlined"
});

//#endregion
export { WeiboOutlined as default };