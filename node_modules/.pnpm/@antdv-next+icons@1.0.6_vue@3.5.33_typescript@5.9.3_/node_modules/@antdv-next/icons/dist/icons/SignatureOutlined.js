import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import SignatureOutlinedSvg from "@ant-design/icons-svg/es/asn/SignatureOutlined.js";

//#region src/icons/SignatureOutlined.tsx
/**![signature](data:image/svg+xml;base64,) */
const SignatureOutlined = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": SignatureOutlinedSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "SignatureOutlined"
});

//#endregion
export { SignatureOutlined as default };