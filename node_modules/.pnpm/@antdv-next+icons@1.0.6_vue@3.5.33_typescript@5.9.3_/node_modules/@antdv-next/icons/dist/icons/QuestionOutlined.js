import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import QuestionOutlinedSvg from "@ant-design/icons-svg/es/asn/QuestionOutlined.js";

//#region src/icons/QuestionOutlined.tsx
/**![question](data:image/svg+xml;base64,) */
const QuestionOutlined = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": QuestionOutlinedSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "QuestionOutlined"
});

//#endregion
export { QuestionOutlined as default };