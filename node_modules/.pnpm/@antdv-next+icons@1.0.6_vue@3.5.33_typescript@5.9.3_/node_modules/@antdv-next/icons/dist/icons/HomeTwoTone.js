import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import HomeTwoToneSvg from "@ant-design/icons-svg/es/asn/HomeTwoTone.js";

//#region src/icons/HomeTwoTone.tsx
/**![home](data:image/svg+xml;base64,) */
const HomeTwoTone = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": HomeTwoToneSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "HomeTwoTone"
});

//#endregion
export { HomeTwoTone as default };