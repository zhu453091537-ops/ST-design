import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import BulbTwoToneSvg from "@ant-design/icons-svg/es/asn/BulbTwoTone.js";

//#region src/icons/BulbTwoTone.tsx
/**![bulb](data:image/svg+xml;base64,) */
const BulbTwoTone = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": BulbTwoToneSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "BulbTwoTone"
});

//#endregion
export { BulbTwoTone as default };