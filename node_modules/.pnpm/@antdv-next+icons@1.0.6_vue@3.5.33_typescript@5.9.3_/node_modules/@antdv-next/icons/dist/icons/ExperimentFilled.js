import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import ExperimentFilledSvg from "@ant-design/icons-svg/es/asn/ExperimentFilled.js";

//#region src/icons/ExperimentFilled.tsx
/**![experiment](data:image/svg+xml;base64,) */
const ExperimentFilled = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": ExperimentFilledSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "ExperimentFilled"
});

//#endregion
export { ExperimentFilled as default };