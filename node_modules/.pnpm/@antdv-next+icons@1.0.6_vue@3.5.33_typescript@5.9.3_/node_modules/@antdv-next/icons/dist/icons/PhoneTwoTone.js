import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import PhoneTwoToneSvg from "@ant-design/icons-svg/es/asn/PhoneTwoTone.js";

//#region src/icons/PhoneTwoTone.tsx
/**![phone](data:image/svg+xml;base64,) */
const PhoneTwoTone = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": PhoneTwoToneSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "PhoneTwoTone"
});

//#endregion
export { PhoneTwoTone as default };