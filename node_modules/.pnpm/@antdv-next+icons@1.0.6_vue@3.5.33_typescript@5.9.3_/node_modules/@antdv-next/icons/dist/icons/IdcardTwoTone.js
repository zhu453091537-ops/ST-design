import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import IdcardTwoToneSvg from "@ant-design/icons-svg/es/asn/IdcardTwoTone.js";

//#region src/icons/IdcardTwoTone.tsx
/**![idcard](data:image/svg+xml;base64,) */
const IdcardTwoTone = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": IdcardTwoToneSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "IdcardTwoTone"
});

//#endregion
export { IdcardTwoTone as default };