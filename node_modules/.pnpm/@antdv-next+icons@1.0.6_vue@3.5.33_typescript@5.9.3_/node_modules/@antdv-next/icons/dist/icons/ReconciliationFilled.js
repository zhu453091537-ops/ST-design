import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import ReconciliationFilledSvg from "@ant-design/icons-svg/es/asn/ReconciliationFilled.js";

//#region src/icons/ReconciliationFilled.tsx
/**![reconciliation](data:image/svg+xml;base64,) */
const ReconciliationFilled = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": ReconciliationFilledSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "ReconciliationFilled"
});

//#endregion
export { ReconciliationFilled as default };