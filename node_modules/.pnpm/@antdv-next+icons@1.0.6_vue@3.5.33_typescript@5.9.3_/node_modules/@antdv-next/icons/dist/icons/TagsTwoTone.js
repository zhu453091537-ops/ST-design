import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import TagsTwoToneSvg from "@ant-design/icons-svg/es/asn/TagsTwoTone.js";

//#region src/icons/TagsTwoTone.tsx
/**![tags](data:image/svg+xml;base64,) */
const TagsTwoTone = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": TagsTwoToneSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "TagsTwoTone"
});

//#endregion
export { TagsTwoTone as default };