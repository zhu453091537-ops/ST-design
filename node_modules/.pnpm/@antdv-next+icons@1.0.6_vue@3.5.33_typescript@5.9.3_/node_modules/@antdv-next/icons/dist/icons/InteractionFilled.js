import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import InteractionFilledSvg from "@ant-design/icons-svg/es/asn/InteractionFilled.js";

//#region src/icons/InteractionFilled.tsx
/**![interaction](data:image/svg+xml;base64,) */
const InteractionFilled = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": InteractionFilledSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "InteractionFilled"
});

//#endregion
export { InteractionFilled as default };