import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import SyncOutlinedSvg from "@ant-design/icons-svg/es/asn/SyncOutlined.js";

//#region src/icons/SyncOutlined.tsx
/**![sync](data:image/svg+xml;base64,) */
const SyncOutlined = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": SyncOutlinedSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "SyncOutlined"
});

//#endregion
export { SyncOutlined as default };