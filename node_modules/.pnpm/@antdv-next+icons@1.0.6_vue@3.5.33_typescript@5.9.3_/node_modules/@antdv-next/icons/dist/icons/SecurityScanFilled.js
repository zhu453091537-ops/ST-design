import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import SecurityScanFilledSvg from "@ant-design/icons-svg/es/asn/SecurityScanFilled.js";

//#region src/icons/SecurityScanFilled.tsx
/**![security-scan](data:image/svg+xml;base64,) */
const SecurityScanFilled = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": SecurityScanFilledSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "SecurityScanFilled"
});

//#endregion
export { SecurityScanFilled as default };