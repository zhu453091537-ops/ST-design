import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import InstagramOutlinedSvg from "@ant-design/icons-svg/es/asn/InstagramOutlined.js";

//#region src/icons/InstagramOutlined.tsx
/**![instagram](data:image/svg+xml;base64,) */
const InstagramOutlined = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": InstagramOutlinedSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "InstagramOutlined"
});

//#endregion
export { InstagramOutlined as default };