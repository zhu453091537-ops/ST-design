import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import TwitterCircleFilledSvg from "@ant-design/icons-svg/es/asn/TwitterCircleFilled.js";

//#region src/icons/TwitterCircleFilled.tsx
/**![twitter-circle](data:image/svg+xml;base64,) */
const TwitterCircleFilled = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": TwitterCircleFilledSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "TwitterCircleFilled"
});

//#endregion
export { TwitterCircleFilled as default };