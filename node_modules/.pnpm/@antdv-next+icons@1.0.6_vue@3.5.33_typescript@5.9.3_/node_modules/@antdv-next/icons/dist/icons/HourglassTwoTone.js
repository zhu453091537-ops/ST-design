import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import HourglassTwoToneSvg from "@ant-design/icons-svg/es/asn/HourglassTwoTone.js";

//#region src/icons/HourglassTwoTone.tsx
/**![hourglass](data:image/svg+xml;base64,) */
const HourglassTwoTone = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": HourglassTwoToneSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "HourglassTwoTone"
});

//#endregion
export { HourglassTwoTone as default };