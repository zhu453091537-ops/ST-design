import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import LinkedinOutlinedSvg from "@ant-design/icons-svg/es/asn/LinkedinOutlined.js";

//#region src/icons/LinkedinOutlined.tsx
/**![linkedin](data:image/svg+xml;base64,) */
const LinkedinOutlined = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": LinkedinOutlinedSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "LinkedinOutlined"
});

//#endregion
export { LinkedinOutlined as default };