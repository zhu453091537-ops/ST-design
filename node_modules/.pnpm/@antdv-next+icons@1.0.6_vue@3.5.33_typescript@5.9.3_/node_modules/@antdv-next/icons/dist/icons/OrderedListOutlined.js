import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import OrderedListOutlinedSvg from "@ant-design/icons-svg/es/asn/OrderedListOutlined.js";

//#region src/icons/OrderedListOutlined.tsx
/**![ordered-list](data:image/svg+xml;base64,) */
const OrderedListOutlined = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": OrderedListOutlinedSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "OrderedListOutlined"
});

//#endregion
export { OrderedListOutlined as default };