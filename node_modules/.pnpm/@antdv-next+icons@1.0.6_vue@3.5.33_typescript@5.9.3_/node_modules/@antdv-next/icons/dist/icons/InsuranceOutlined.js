import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import InsuranceOutlinedSvg from "@ant-design/icons-svg/es/asn/InsuranceOutlined.js";

//#region src/icons/InsuranceOutlined.tsx
/**![insurance](data:image/svg+xml;base64,) */
const InsuranceOutlined = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": InsuranceOutlinedSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "InsuranceOutlined"
});

//#endregion
export { InsuranceOutlined as default };