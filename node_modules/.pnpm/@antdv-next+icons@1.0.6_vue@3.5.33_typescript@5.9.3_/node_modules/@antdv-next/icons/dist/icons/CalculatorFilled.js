import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import CalculatorFilledSvg from "@ant-design/icons-svg/es/asn/CalculatorFilled.js";

//#region src/icons/CalculatorFilled.tsx
/**![calculator](data:image/svg+xml;base64,) */
const CalculatorFilled = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": CalculatorFilledSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "CalculatorFilled"
});

//#endregion
export { CalculatorFilled as default };