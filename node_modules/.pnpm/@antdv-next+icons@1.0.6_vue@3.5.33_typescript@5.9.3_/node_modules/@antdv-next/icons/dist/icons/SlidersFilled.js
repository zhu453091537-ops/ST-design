import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import SlidersFilledSvg from "@ant-design/icons-svg/es/asn/SlidersFilled.js";

//#region src/icons/SlidersFilled.tsx
/**![sliders](data:image/svg+xml;base64,) */
const SlidersFilled = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": SlidersFilledSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "SlidersFilled"
});

//#endregion
export { SlidersFilled as default };