import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import TwitterSquareFilledSvg from "@ant-design/icons-svg/es/asn/TwitterSquareFilled.js";

//#region src/icons/TwitterSquareFilled.tsx
/**![twitter-square](data:image/svg+xml;base64,) */
const TwitterSquareFilled = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": TwitterSquareFilledSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "TwitterSquareFilled"
});

//#endregion
export { TwitterSquareFilled as default };