import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import YuqueOutlinedSvg from "@ant-design/icons-svg/es/asn/YuqueOutlined.js";

//#region src/icons/YuqueOutlined.tsx
/**![yuque](data:image/svg+xml;base64,) */
const YuqueOutlined = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": YuqueOutlinedSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "YuqueOutlined"
});

//#endregion
export { YuqueOutlined as default };