import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import TransactionOutlinedSvg from "@ant-design/icons-svg/es/asn/TransactionOutlined.js";

//#region src/icons/TransactionOutlined.tsx
/**![transaction](data:image/svg+xml;base64,) */
const TransactionOutlined = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": TransactionOutlinedSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "TransactionOutlined"
});

//#endregion
export { TransactionOutlined as default };