import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import TaobaoOutlinedSvg from "@ant-design/icons-svg/es/asn/TaobaoOutlined.js";

//#region src/icons/TaobaoOutlined.tsx
/**![taobao](data:image/svg+xml;base64,) */
const TaobaoOutlined = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": TaobaoOutlinedSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "TaobaoOutlined"
});

//#endregion
export { TaobaoOutlined as default };