import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import Html5TwoToneSvg from "@ant-design/icons-svg/es/asn/Html5TwoTone.js";

//#region src/icons/Html5TwoTone.tsx
/**![html5](data:image/svg+xml;base64,) */
const Html5TwoTone = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": Html5TwoToneSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "Html5TwoTone"
});

//#endregion
export { Html5TwoTone as default };