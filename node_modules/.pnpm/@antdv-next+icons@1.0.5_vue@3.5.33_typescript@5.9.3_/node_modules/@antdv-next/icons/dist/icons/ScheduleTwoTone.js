import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import ScheduleTwoToneSvg from "@ant-design/icons-svg/es/asn/ScheduleTwoTone.js";

//#region src/icons/ScheduleTwoTone.tsx
/**![schedule](data:image/svg+xml;base64,) */
const ScheduleTwoTone = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": ScheduleTwoToneSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "ScheduleTwoTone"
});

//#endregion
export { ScheduleTwoTone as default };