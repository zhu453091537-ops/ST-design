import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import MedicineBoxFilledSvg from "@ant-design/icons-svg/es/asn/MedicineBoxFilled.js";

//#region src/icons/MedicineBoxFilled.tsx
/**![medicine-box](data:image/svg+xml;base64,) */
const MedicineBoxFilled = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": MedicineBoxFilledSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "MedicineBoxFilled"
});

//#endregion
export { MedicineBoxFilled as default };