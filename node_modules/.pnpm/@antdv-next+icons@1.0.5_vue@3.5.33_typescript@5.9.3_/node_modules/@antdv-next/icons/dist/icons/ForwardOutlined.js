import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import ForwardOutlinedSvg from "@ant-design/icons-svg/es/asn/ForwardOutlined.js";

//#region src/icons/ForwardOutlined.tsx
/**![forward](data:image/svg+xml;base64,) */
const ForwardOutlined = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": ForwardOutlinedSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "ForwardOutlined"
});

//#endregion
export { ForwardOutlined as default };