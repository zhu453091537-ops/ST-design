import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import DisconnectOutlinedSvg from "@ant-design/icons-svg/es/asn/DisconnectOutlined.js";

//#region src/icons/DisconnectOutlined.tsx
/**![disconnect](data:image/svg+xml;base64,) */
const DisconnectOutlined = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": DisconnectOutlinedSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "DisconnectOutlined"
});

//#endregion
export { DisconnectOutlined as default };