import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import CommentOutlinedSvg from "@ant-design/icons-svg/es/asn/CommentOutlined.js";

//#region src/icons/CommentOutlined.tsx
/**![comment](data:image/svg+xml;base64,) */
const CommentOutlined = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": CommentOutlinedSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "CommentOutlined"
});

//#endregion
export { CommentOutlined as default };