import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import ScheduleFilledSvg from "@ant-design/icons-svg/es/asn/ScheduleFilled.js";

//#region src/icons/ScheduleFilled.tsx
/**![schedule](data:image/svg+xml;base64,) */
const ScheduleFilled = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": ScheduleFilledSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "ScheduleFilled"
});

//#endregion
export { ScheduleFilled as default };