import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import CiCircleTwoToneSvg from "@ant-design/icons-svg/es/asn/CiCircleTwoTone.js";

//#region src/icons/CiCircleTwoTone.tsx
/**![ci-circle](data:image/svg+xml;base64,) */
const CiCircleTwoTone = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": CiCircleTwoToneSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "CiCircleTwoTone"
});

//#endregion
export { CiCircleTwoTone as default };