import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import TrophyFilledSvg from "@ant-design/icons-svg/es/asn/TrophyFilled.js";

//#region src/icons/TrophyFilled.tsx
/**![trophy](data:image/svg+xml;base64,) */
const TrophyFilled = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": TrophyFilledSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "TrophyFilled"
});

//#endregion
export { TrophyFilled as default };