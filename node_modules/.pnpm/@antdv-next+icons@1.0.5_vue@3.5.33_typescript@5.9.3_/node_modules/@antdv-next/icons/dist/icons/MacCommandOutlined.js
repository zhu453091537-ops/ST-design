import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import MacCommandOutlinedSvg from "@ant-design/icons-svg/es/asn/MacCommandOutlined.js";

//#region src/icons/MacCommandOutlined.tsx
/**![mac-command](data:image/svg+xml;base64,) */
const MacCommandOutlined = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": MacCommandOutlinedSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "MacCommandOutlined"
});

//#endregion
export { MacCommandOutlined as default };