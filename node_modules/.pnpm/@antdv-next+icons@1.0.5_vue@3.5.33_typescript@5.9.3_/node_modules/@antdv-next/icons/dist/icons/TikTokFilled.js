import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import TikTokFilledSvg from "@ant-design/icons-svg/es/asn/TikTokFilled.js";

//#region src/icons/TikTokFilled.tsx
/**![tik-tok](data:image/svg+xml;base64,) */
const TikTokFilled = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": TikTokFilledSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "TikTokFilled"
});

//#endregion
export { TikTokFilled as default };