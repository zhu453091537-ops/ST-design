import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import ZhihuOutlinedSvg from "@ant-design/icons-svg/es/asn/ZhihuOutlined.js";

//#region src/icons/ZhihuOutlined.tsx
/**![zhihu](data:image/svg+xml;base64,) */
const ZhihuOutlined = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": ZhihuOutlinedSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "ZhihuOutlined"
});

//#endregion
export { ZhihuOutlined as default };