import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import MediumCircleFilledSvg from "@ant-design/icons-svg/es/asn/MediumCircleFilled.js";

//#region src/icons/MediumCircleFilled.tsx
/**![medium-circle](data:image/svg+xml;base64,) */
const MediumCircleFilled = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": MediumCircleFilledSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "MediumCircleFilled"
});

//#endregion
export { MediumCircleFilled as default };