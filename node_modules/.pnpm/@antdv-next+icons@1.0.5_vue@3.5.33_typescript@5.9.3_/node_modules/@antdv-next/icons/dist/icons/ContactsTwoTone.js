import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import ContactsTwoToneSvg from "@ant-design/icons-svg/es/asn/ContactsTwoTone.js";

//#region src/icons/ContactsTwoTone.tsx
/**![contacts](data:image/svg+xml;base64,) */
const ContactsTwoTone = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": ContactsTwoToneSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "ContactsTwoTone"
});

//#endregion
export { ContactsTwoTone as default };