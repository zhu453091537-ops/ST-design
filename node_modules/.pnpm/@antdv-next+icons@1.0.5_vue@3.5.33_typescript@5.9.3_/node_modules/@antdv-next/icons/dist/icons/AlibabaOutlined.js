import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import AlibabaOutlinedSvg from "@ant-design/icons-svg/es/asn/AlibabaOutlined.js";

//#region src/icons/AlibabaOutlined.tsx
/**![alibaba](data:image/svg+xml;base64,) */
const AlibabaOutlined = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": AlibabaOutlinedSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "AlibabaOutlined"
});

//#endregion
export { AlibabaOutlined as default };