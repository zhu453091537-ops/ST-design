import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import FileExcelOutlinedSvg from "@ant-design/icons-svg/es/asn/FileExcelOutlined.js";

//#region src/icons/FileExcelOutlined.tsx
/**![file-excel](data:image/svg+xml;base64,) */
const FileExcelOutlined = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": FileExcelOutlinedSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "FileExcelOutlined"
});

//#endregion
export { FileExcelOutlined as default };