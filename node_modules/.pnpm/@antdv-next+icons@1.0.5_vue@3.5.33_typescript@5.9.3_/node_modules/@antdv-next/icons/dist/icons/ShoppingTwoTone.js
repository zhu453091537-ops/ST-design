import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import ShoppingTwoToneSvg from "@ant-design/icons-svg/es/asn/ShoppingTwoTone.js";

//#region src/icons/ShoppingTwoTone.tsx
/**![shopping](data:image/svg+xml;base64,) */
const ShoppingTwoTone = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": ShoppingTwoToneSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "ShoppingTwoTone"
});

//#endregion
export { ShoppingTwoTone as default };