import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import BgColorsOutlinedSvg from "@ant-design/icons-svg/es/asn/BgColorsOutlined.js";

//#region src/icons/BgColorsOutlined.tsx
/**![bg-colors](data:image/svg+xml;base64,) */
const BgColorsOutlined = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": BgColorsOutlinedSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "BgColorsOutlined"
});

//#endregion
export { BgColorsOutlined as default };