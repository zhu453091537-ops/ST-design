import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import ExperimentTwoToneSvg from "@ant-design/icons-svg/es/asn/ExperimentTwoTone.js";

//#region src/icons/ExperimentTwoTone.tsx
/**![experiment](data:image/svg+xml;base64,) */
const ExperimentTwoTone = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": ExperimentTwoToneSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "ExperimentTwoTone"
});

//#endregion
export { ExperimentTwoTone as default };