import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import WeiboCircleFilledSvg from "@ant-design/icons-svg/es/asn/WeiboCircleFilled.js";

//#region src/icons/WeiboCircleFilled.tsx
/**![weibo-circle](data:image/svg+xml;base64,) */
const WeiboCircleFilled = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": WeiboCircleFilledSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "WeiboCircleFilled"
});

//#endregion
export { WeiboCircleFilled as default };