import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import SkypeFilledSvg from "@ant-design/icons-svg/es/asn/SkypeFilled.js";

//#region src/icons/SkypeFilled.tsx
/**![skype](data:image/svg+xml;base64,) */
const SkypeFilled = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": SkypeFilledSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "SkypeFilled"
});

//#endregion
export { SkypeFilled as default };