import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import InsuranceFilledSvg from "@ant-design/icons-svg/es/asn/InsuranceFilled.js";

//#region src/icons/InsuranceFilled.tsx
/**![insurance](data:image/svg+xml;base64,) */
const InsuranceFilled = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": InsuranceFilledSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "InsuranceFilled"
});

//#endregion
export { InsuranceFilled as default };