import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import AppleOutlinedSvg from "@ant-design/icons-svg/es/asn/AppleOutlined.js";

//#region src/icons/AppleOutlined.tsx
/**![apple](data:image/svg+xml;base64,) */
const AppleOutlined = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": AppleOutlinedSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "AppleOutlined"
});

//#endregion
export { AppleOutlined as default };