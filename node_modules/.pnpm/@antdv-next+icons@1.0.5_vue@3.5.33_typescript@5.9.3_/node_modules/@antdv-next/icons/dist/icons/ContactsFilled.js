import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import ContactsFilledSvg from "@ant-design/icons-svg/es/asn/ContactsFilled.js";

//#region src/icons/ContactsFilled.tsx
/**![contacts](data:image/svg+xml;base64,) */
const ContactsFilled = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": ContactsFilledSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "ContactsFilled"
});

//#endregion
export { ContactsFilled as default };