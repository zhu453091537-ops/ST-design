import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import TeamOutlinedSvg from "@ant-design/icons-svg/es/asn/TeamOutlined.js";

//#region src/icons/TeamOutlined.tsx
/**![team](data:image/svg+xml;base64,) */
const TeamOutlined = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": TeamOutlinedSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "TeamOutlined"
});

//#endregion
export { TeamOutlined as default };