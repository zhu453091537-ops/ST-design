import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import FileTextTwoToneSvg from "@ant-design/icons-svg/es/asn/FileTextTwoTone.js";

//#region src/icons/FileTextTwoTone.tsx
/**![file-text](data:image/svg+xml;base64,) */
const FileTextTwoTone = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": FileTextTwoToneSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "FileTextTwoTone"
});

//#endregion
export { FileTextTwoTone as default };