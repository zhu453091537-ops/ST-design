import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import FileExcelTwoToneSvg from "@ant-design/icons-svg/es/asn/FileExcelTwoTone.js";

//#region src/icons/FileExcelTwoTone.tsx
/**![file-excel](data:image/svg+xml;base64,) */
const FileExcelTwoTone = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": FileExcelTwoToneSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "FileExcelTwoTone"
});

//#endregion
export { FileExcelTwoTone as default };