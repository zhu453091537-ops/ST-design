import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import AppstoreFilledSvg from "@ant-design/icons-svg/es/asn/AppstoreFilled.js";

//#region src/icons/AppstoreFilled.tsx
/**![appstore](data:image/svg+xml;base64,) */
const AppstoreFilled = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": AppstoreFilledSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "AppstoreFilled"
});

//#endregion
export { AppstoreFilled as default };