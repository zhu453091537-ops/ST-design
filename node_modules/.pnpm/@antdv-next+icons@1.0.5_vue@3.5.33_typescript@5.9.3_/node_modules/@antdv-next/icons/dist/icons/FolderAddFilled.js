import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import FolderAddFilledSvg from "@ant-design/icons-svg/es/asn/FolderAddFilled.js";

//#region src/icons/FolderAddFilled.tsx
/**![folder-add](data:image/svg+xml;base64,) */
const FolderAddFilled = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": FolderAddFilledSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "FolderAddFilled"
});

//#endregion
export { FolderAddFilled as default };