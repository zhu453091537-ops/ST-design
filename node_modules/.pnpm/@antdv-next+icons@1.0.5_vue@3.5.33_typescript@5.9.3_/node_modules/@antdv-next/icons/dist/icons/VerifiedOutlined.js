import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import VerifiedOutlinedSvg from "@ant-design/icons-svg/es/asn/VerifiedOutlined.js";

//#region src/icons/VerifiedOutlined.tsx
/**![verified](data:image/svg+xml;base64,) */
const VerifiedOutlined = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": VerifiedOutlinedSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "VerifiedOutlined"
});

//#endregion
export { VerifiedOutlined as default };