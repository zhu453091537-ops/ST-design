import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import SlidersTwoToneSvg from "@ant-design/icons-svg/es/asn/SlidersTwoTone.js";

//#region src/icons/SlidersTwoTone.tsx
/**![sliders](data:image/svg+xml;base64,) */
const SlidersTwoTone = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": SlidersTwoToneSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "SlidersTwoTone"
});

//#endregion
export { SlidersTwoTone as default };