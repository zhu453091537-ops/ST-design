import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import WeiboSquareFilledSvg from "@ant-design/icons-svg/es/asn/WeiboSquareFilled.js";

//#region src/icons/WeiboSquareFilled.tsx
/**![weibo-square](data:image/svg+xml;base64,) */
const WeiboSquareFilled = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": WeiboSquareFilledSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "WeiboSquareFilled"
});

//#endregion
export { WeiboSquareFilled as default };