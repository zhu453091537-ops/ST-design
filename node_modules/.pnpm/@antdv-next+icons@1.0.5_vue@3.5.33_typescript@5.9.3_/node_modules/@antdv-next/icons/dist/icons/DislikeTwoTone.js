import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import DislikeTwoToneSvg from "@ant-design/icons-svg/es/asn/DislikeTwoTone.js";

//#region src/icons/DislikeTwoTone.tsx
/**![dislike](data:image/svg+xml;base64,) */
const DislikeTwoTone = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": DislikeTwoToneSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "DislikeTwoTone"
});

//#endregion
export { DislikeTwoTone as default };