import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import PoweroffOutlinedSvg from "@ant-design/icons-svg/es/asn/PoweroffOutlined.js";

//#region src/icons/PoweroffOutlined.tsx
/**![poweroff](data:image/svg+xml;base64,) */
const PoweroffOutlined = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": PoweroffOutlinedSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "PoweroffOutlined"
});

//#endregion
export { PoweroffOutlined as default };