import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import AliwangwangFilledSvg from "@ant-design/icons-svg/es/asn/AliwangwangFilled.js";

//#region src/icons/AliwangwangFilled.tsx
/**![aliwangwang](data:image/svg+xml;base64,) */
const AliwangwangFilled = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": AliwangwangFilledSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "AliwangwangFilled"
});

//#endregion
export { AliwangwangFilled as default };