import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import FastBackwardFilledSvg from "@ant-design/icons-svg/es/asn/FastBackwardFilled.js";

//#region src/icons/FastBackwardFilled.tsx
/**![fast-backward](data:image/svg+xml;base64,) */
const FastBackwardFilled = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": FastBackwardFilledSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "FastBackwardFilled"
});

//#endregion
export { FastBackwardFilled as default };