import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import ShopTwoToneSvg from "@ant-design/icons-svg/es/asn/ShopTwoTone.js";

//#region src/icons/ShopTwoTone.tsx
/**![shop](data:image/svg+xml;base64,) */
const ShopTwoTone = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": ShopTwoToneSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "ShopTwoTone"
});

//#endregion
export { ShopTwoTone as default };