import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import PullRequestOutlinedSvg from "@ant-design/icons-svg/es/asn/PullRequestOutlined.js";

//#region src/icons/PullRequestOutlined.tsx
/**![pull-request](data:image/svg+xml;base64,) */
const PullRequestOutlined = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": PullRequestOutlinedSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "PullRequestOutlined"
});

//#endregion
export { PullRequestOutlined as default };