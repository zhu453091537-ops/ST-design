import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import ScheduleOutlinedSvg from "@ant-design/icons-svg/es/asn/ScheduleOutlined.js";

//#region src/icons/ScheduleOutlined.tsx
/**![schedule](data:image/svg+xml;base64,) */
const ScheduleOutlined = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": ScheduleOutlinedSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "ScheduleOutlined"
});

//#endregion
export { ScheduleOutlined as default };