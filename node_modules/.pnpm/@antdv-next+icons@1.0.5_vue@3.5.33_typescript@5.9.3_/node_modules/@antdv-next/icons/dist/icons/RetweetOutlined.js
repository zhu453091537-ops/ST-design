import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import RetweetOutlinedSvg from "@ant-design/icons-svg/es/asn/RetweetOutlined.js";

//#region src/icons/RetweetOutlined.tsx
/**![retweet](data:image/svg+xml;base64,) */
const RetweetOutlined = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": RetweetOutlinedSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "RetweetOutlined"
});

//#endregion
export { RetweetOutlined as default };