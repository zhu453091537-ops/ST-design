import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import WechatOutlinedSvg from "@ant-design/icons-svg/es/asn/WechatOutlined.js";

//#region src/icons/WechatOutlined.tsx
/**![wechat](data:image/svg+xml;base64,) */
const WechatOutlined = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": WechatOutlinedSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "WechatOutlined"
});

//#endregion
export { WechatOutlined as default };