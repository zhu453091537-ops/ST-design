import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import PieChartFilledSvg from "@ant-design/icons-svg/es/asn/PieChartFilled.js";

//#region src/icons/PieChartFilled.tsx
/**![pie-chart](data:image/svg+xml;base64,) */
const PieChartFilled = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": PieChartFilledSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "PieChartFilled"
});

//#endregion
export { PieChartFilled as default };