import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import HddTwoToneSvg from "@ant-design/icons-svg/es/asn/HddTwoTone.js";

//#region src/icons/HddTwoTone.tsx
/**![hdd](data:image/svg+xml;base64,) */
const HddTwoTone = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": HddTwoToneSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "HddTwoTone"
});

//#endregion
export { HddTwoTone as default };