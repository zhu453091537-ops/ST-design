import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import SaveTwoToneSvg from "@ant-design/icons-svg/es/asn/SaveTwoTone.js";

//#region src/icons/SaveTwoTone.tsx
/**![save](data:image/svg+xml;base64,) */
const SaveTwoTone = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": SaveTwoToneSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "SaveTwoTone"
});

//#endregion
export { SaveTwoTone as default };