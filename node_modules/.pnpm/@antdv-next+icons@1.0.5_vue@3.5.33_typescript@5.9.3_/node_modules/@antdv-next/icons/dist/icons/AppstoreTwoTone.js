import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import AppstoreTwoToneSvg from "@ant-design/icons-svg/es/asn/AppstoreTwoTone.js";

//#region src/icons/AppstoreTwoTone.tsx
/**![appstore](data:image/svg+xml;base64,) */
const AppstoreTwoTone = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": AppstoreTwoToneSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "AppstoreTwoTone"
});

//#endregion
export { AppstoreTwoTone as default };