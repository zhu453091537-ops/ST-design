import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import PinterestFilledSvg from "@ant-design/icons-svg/es/asn/PinterestFilled.js";

//#region src/icons/PinterestFilled.tsx
/**![pinterest](data:image/svg+xml;base64,) */
const PinterestFilled = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": PinterestFilledSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "PinterestFilled"
});

//#endregion
export { PinterestFilled as default };