import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import SignatureFilledSvg from "@ant-design/icons-svg/es/asn/SignatureFilled.js";

//#region src/icons/SignatureFilled.tsx
/**![signature](data:image/svg+xml;base64,) */
const SignatureFilled = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": SignatureFilledSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "SignatureFilled"
});

//#endregion
export { SignatureFilled as default };