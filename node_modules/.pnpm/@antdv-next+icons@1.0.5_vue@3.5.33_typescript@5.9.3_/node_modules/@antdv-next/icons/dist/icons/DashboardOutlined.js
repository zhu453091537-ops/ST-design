import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import DashboardOutlinedSvg from "@ant-design/icons-svg/es/asn/DashboardOutlined.js";

//#region src/icons/DashboardOutlined.tsx
/**![dashboard](data:image/svg+xml;base64,) */
const DashboardOutlined = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": DashboardOutlinedSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "DashboardOutlined"
});

//#endregion
export { DashboardOutlined as default };