import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import AntDesignOutlinedSvg from "@ant-design/icons-svg/es/asn/AntDesignOutlined.js";

//#region src/icons/AntDesignOutlined.tsx
/**![ant-design](data:image/svg+xml;base64,) */
const AntDesignOutlined = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": AntDesignOutlinedSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "AntDesignOutlined"
});

//#endregion
export { AntDesignOutlined as default };