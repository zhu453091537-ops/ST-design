import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import CustomerServiceFilledSvg from "@ant-design/icons-svg/es/asn/CustomerServiceFilled.js";

//#region src/icons/CustomerServiceFilled.tsx
/**![customer-service](data:image/svg+xml;base64,) */
const CustomerServiceFilled = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": CustomerServiceFilledSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "CustomerServiceFilled"
});

//#endregion
export { CustomerServiceFilled as default };