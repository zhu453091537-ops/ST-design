import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import DiffTwoToneSvg from "@ant-design/icons-svg/es/asn/DiffTwoTone.js";

//#region src/icons/DiffTwoTone.tsx
/**![diff](data:image/svg+xml;base64,) */
const DiffTwoTone = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": DiffTwoToneSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "DiffTwoTone"
});

//#endregion
export { DiffTwoTone as default };