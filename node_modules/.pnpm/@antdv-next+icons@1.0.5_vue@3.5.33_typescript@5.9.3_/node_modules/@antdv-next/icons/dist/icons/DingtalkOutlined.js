import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import DingtalkOutlinedSvg from "@ant-design/icons-svg/es/asn/DingtalkOutlined.js";

//#region src/icons/DingtalkOutlined.tsx
/**![dingtalk](data:image/svg+xml;base64,) */
const DingtalkOutlined = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": DingtalkOutlinedSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "DingtalkOutlined"
});

//#endregion
export { DingtalkOutlined as default };