import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import FastForwardOutlinedSvg from "@ant-design/icons-svg/es/asn/FastForwardOutlined.js";

//#region src/icons/FastForwardOutlined.tsx
/**![fast-forward](data:image/svg+xml;base64,) */
const FastForwardOutlined = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": FastForwardOutlinedSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "FastForwardOutlined"
});

//#endregion
export { FastForwardOutlined as default };