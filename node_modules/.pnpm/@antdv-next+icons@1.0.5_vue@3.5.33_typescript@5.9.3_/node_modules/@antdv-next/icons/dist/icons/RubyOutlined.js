import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import RubyOutlinedSvg from "@ant-design/icons-svg/es/asn/RubyOutlined.js";

//#region src/icons/RubyOutlined.tsx
/**![ruby](data:image/svg+xml;base64,) */
const RubyOutlined = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": RubyOutlinedSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "RubyOutlined"
});

//#endregion
export { RubyOutlined as default };