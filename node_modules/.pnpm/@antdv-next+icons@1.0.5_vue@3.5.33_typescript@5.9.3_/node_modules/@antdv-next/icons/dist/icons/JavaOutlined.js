import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import JavaOutlinedSvg from "@ant-design/icons-svg/es/asn/JavaOutlined.js";

//#region src/icons/JavaOutlined.tsx
/**![java](data:image/svg+xml;base64,) */
const JavaOutlined = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": JavaOutlinedSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "JavaOutlined"
});

//#endregion
export { JavaOutlined as default };