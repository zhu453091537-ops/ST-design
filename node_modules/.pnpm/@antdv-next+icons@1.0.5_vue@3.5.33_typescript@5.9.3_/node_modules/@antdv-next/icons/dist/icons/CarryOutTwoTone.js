import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import CarryOutTwoToneSvg from "@ant-design/icons-svg/es/asn/CarryOutTwoTone.js";

//#region src/icons/CarryOutTwoTone.tsx
/**![carry-out](data:image/svg+xml;base64,) */
const CarryOutTwoTone = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": CarryOutTwoToneSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "CarryOutTwoTone"
});

//#endregion
export { CarryOutTwoTone as default };