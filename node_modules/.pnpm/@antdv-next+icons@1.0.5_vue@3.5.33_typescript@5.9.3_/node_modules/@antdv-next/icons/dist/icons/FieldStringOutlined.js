import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import FieldStringOutlinedSvg from "@ant-design/icons-svg/es/asn/FieldStringOutlined.js";

//#region src/icons/FieldStringOutlined.tsx
/**![field-string](data:image/svg+xml;base64,) */
const FieldStringOutlined = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": FieldStringOutlinedSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "FieldStringOutlined"
});

//#endregion
export { FieldStringOutlined as default };