import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import BaiduOutlinedSvg from "@ant-design/icons-svg/es/asn/BaiduOutlined.js";

//#region src/icons/BaiduOutlined.tsx
/**![baidu](data:image/svg+xml;base64,) */
const BaiduOutlined = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": BaiduOutlinedSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "BaiduOutlined"
});

//#endregion
export { BaiduOutlined as default };