import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import ThunderboltFilledSvg from "@ant-design/icons-svg/es/asn/ThunderboltFilled.js";

//#region src/icons/ThunderboltFilled.tsx
/**![thunderbolt](data:image/svg+xml;base64,) */
const ThunderboltFilled = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": ThunderboltFilledSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "ThunderboltFilled"
});

//#endregion
export { ThunderboltFilled as default };