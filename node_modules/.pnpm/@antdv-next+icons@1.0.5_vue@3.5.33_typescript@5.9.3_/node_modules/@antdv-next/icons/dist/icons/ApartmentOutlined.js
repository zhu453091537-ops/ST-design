import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import ApartmentOutlinedSvg from "@ant-design/icons-svg/es/asn/ApartmentOutlined.js";

//#region src/icons/ApartmentOutlined.tsx
/**![apartment](data:image/svg+xml;base64,) */
const ApartmentOutlined = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": ApartmentOutlinedSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "ApartmentOutlined"
});

//#endregion
export { ApartmentOutlined as default };