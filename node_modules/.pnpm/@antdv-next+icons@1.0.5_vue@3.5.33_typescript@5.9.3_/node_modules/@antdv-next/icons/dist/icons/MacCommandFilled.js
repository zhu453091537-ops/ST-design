import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import MacCommandFilledSvg from "@ant-design/icons-svg/es/asn/MacCommandFilled.js";

//#region src/icons/MacCommandFilled.tsx
/**![mac-command](data:image/svg+xml;base64,) */
const MacCommandFilled = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": MacCommandFilledSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "MacCommandFilled"
});

//#endregion
export { MacCommandFilled as default };