import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import OpenAIFilledSvg from "@ant-design/icons-svg/es/asn/OpenAIFilled.js";

//#region src/icons/OpenAIFilled.tsx
/**![open-a-i](data:image/svg+xml;base64,) */
const OpenAIFilled = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": OpenAIFilledSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "OpenAIFilled"
});

//#endregion
export { OpenAIFilled as default };