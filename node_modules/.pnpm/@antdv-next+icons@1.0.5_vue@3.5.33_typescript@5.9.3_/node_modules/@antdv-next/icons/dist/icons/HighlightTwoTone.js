import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import HighlightTwoToneSvg from "@ant-design/icons-svg/es/asn/HighlightTwoTone.js";

//#region src/icons/HighlightTwoTone.tsx
/**![highlight](data:image/svg+xml;base64,) */
const HighlightTwoTone = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": HighlightTwoToneSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "HighlightTwoTone"
});

//#endregion
export { HighlightTwoTone as default };