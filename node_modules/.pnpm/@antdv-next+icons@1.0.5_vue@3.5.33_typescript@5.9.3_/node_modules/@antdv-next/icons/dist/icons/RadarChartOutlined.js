import AntdIcon from "../components/AntdIcon.js";
import { createVNode, defineComponent, mergeProps } from "vue";
import RadarChartOutlinedSvg from "@ant-design/icons-svg/es/asn/RadarChartOutlined.js";

//#region src/icons/RadarChartOutlined.tsx
/**![radar-chart](data:image/svg+xml;base64,) */
const RadarChartOutlined = /* @__PURE__ */ defineComponent((props) => {
	return () => {
		return createVNode(AntdIcon, mergeProps(props, { "icon": RadarChartOutlinedSvg }), null);
	};
}, {
	props: {
		twoToneColor: {
			type: [String, Array],
			required: false
		},
		onClick: {
			type: Function,
			required: false
		},
		tabIndex: {
			type: Number,
			required: false
		},
		spin: {
			type: Boolean,
			required: false
		},
		rotate: {
			type: Number,
			required: false
		}
	},
	name: "RadarChartOutlined"
});

//#endregion
export { RadarChartOutlined as default };