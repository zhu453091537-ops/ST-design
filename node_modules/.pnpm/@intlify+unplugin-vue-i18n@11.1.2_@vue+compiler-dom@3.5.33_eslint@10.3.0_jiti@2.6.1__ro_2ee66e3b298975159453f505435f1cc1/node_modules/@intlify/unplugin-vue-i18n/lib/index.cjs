...
const debug$3 = createDebug__default(resolveNamespace("resource"));
      ...
      async handler(code, id) {
        ...
        if (!query.vue) {
          ...
          if (isCustomBlock(query)) {
            ...
            const generate = getGenerator(langInfo, bundleUtils.generateYAML);
...
const debug$2 = createDebug__default(resolveNamespace("directive"));
const tsEstree = {
  parse: void 0,
...
const debug$1 = createDebug__default(resolveNamespace("tree-shaking"));
function treeShakingPlugin(resolvedOptions, collector) {
  ...
  return {
    ...
    async buildStart() {
      ...
      const files = await fg__default(patterns, {
        ignore: ["**/node_modules/**"],
        absolute: true
      });
      ...
      for (const file of files) {
        try {
          const content = await node_fs.promises.readFile(file, "utf-8");
