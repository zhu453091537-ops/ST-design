...
function tryGetPackageInfo(name) {
  if (process.versions.pnp) {
    const targetRequire = require$$0.createRequire((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('shared/unplugin-dts.BSDVFQ9C.cjs', document.baseURI).href)));
    try {
...
class Runtime {
  ...
  constructor(options, { createParsedCommandLine, createProgram }) {
    ...
    let publicRoot = compilerOptions.rootDir ? ensureAbsolute(resolveConfigDir(compilerOptions.rootDir, root), root) : compilerOptions.composite && compilerOptions.configFilePath ? node_path.dirname(compilerOptions.configFilePath) : compareVersions.compare(ts.version, "6.0.0", ">=") ? configPath ? node_path.dirname(configPath) : root : queryPublicPath(
    ...
    await runParallel(
      ...
      async ([filePath, content]) => {
        const newFilePath = resolve(
          outDir,
          node_path.relative(entryRoot, cleanVueFileName ? cleanVueDtsFileName(filePath) : filePath)
        );
        if (content) {
...
`
      ...
      for (const name of entryNames) {
        ...
        if (transformed.has(node_path.relative(outDir, entryDtsPath)) && node_fs.existsSync(entryDtsPath)) continue;
