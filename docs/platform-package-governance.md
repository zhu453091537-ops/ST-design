...
5. `apps/web-antd/src/adapter/vxe-table.ts` 仍保留应用侧初始化职责，包括 `useVbenForm` 注入和 `CellImage` / `CellLink` 等当前应用渲染器注册。
6. 真实 token、ECharts、Upload 等实现仍需后续按页面和验证范围分批迁移，不在本阶段一次性搬空。
