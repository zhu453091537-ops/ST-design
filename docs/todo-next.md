...
| 优先级 | 事项 | 状态 | 说明 |
| --- | --- | --- | --- |
| 高 | 平台组件 workspace 包化迁移 | 约 99%，应用引用、Antdv 样式入口、Vxe 适配入口和 Vxe 样式入口已收口，Vite 构建通过 | 已明确不使用手工 symlink 和多份组件拷贝；平台组件源码已迁入 `packages/platform-ui/src/**`，应用源码中的平台组件导入已统一为 `@st/platform-ui`，`apps/web-antd/src/components/platform/index.ts` 仅保留兼容出口。`@st/platform-styles`、`@st/platform-adapter`、`@st/platform-types`、`@st/platform-utils` 四个包骨架已补齐，`apps/web-antd` 运行时样式入口已切到 `@st/platform-styles/antd`，该入口已承载 Ant Design Vue 全局覆盖；Vxe 适配入口已切到 `@st/platform-adapter/vxe-table`。`platform-adapter/vxe-table` 已开始承载默认序号列、工具栏默认项、刷新方式、复选状态和排序参数转换；`@st/platform-styles/vxe-table` 已开始承载稳定 Vxe 表格变量和全局覆盖。当前 Vite build 已通过，类型检查仍剩历史组件 `tree-select-panel.vue` 的 `antdv-next/es/checkbox/interface` 深路径声明问题。下一步继续做 ECharts / Upload 适配实现的分阶段迁移，并在本地服务可访问后抽查真实 Vxe 页。 |
